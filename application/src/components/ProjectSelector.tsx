...
import { Plus, Database, FileSpreadsheet, FolderOpen, ArrowRight, Trash2, Loader2, AlertCircle } from 'lucide-react';
...
export const ProjectSelector: React.FC<ProjectSelectorProps> = ({ onSelectProject }) => {
    ...
    const fetchProjects = async () => {
        ...
        try {
            ...
            if (e.message?.includes('initializing') || e.message?.includes('503')) {
                setError('DATABASE_INITIALIZING');
            } else {
    ...
    useEffect(() => {
        fetchProjects();
        // Poll every 5 seconds if initializing
        const interval = setInterval(() => {
            if (error === 'DATABASE_INITIALIZING' || loading) {
                fetchProjects();
            }
        }, 5000);
    ...
    if (error === 'DATABASE_INITIALIZING' && !apiService.isLocalMode()) {
        return (
            <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f8fafc', padding: 20, textAlign: 'center' }}>
                <div style={{ background: 'white', padding: '40px', borderRadius: 16, boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', maxWidth: 400 }}>
                    <div style={{ position: 'relative', width: 64, height: 64, margin: '0 auto 24px' }}>
                        <Database size={64} style={{ color: '#3b82f6', opacity: 0.2 }} />
                        <Loader2 size={32} style={{ color: '#3b82f6', position: 'absolute', top: 16, left: 16, animation: 'spin 2s linear infinite' }} />
                    </div>
                    <h2 style={{ fontSize: 20, color: '#0f172a', marginBottom: 12 }}>Connecting to Fabric</h2>
                    <p style={{ color: '#64748b', fontSize: 14, lineHeight: 1.6, marginBottom: 24 }}>
                        The backend is currently authenticating with Microsoft Fabric. Please check your terminal for the login code.
                    </p>
                    <div style={{ fontSize: 12, color: '#94a3b8', background: '#f1f5f9', padding: '12px', borderRadius: 8 }}>
    ...
    return (
        <div style={{ height: '100vh', overflowY: 'auto', background: '#f8fafc', padding: '40px 20px', fontFamily: 'system-ui, sans-serif' }}>
            <div style={{ maxWidth: 1000, margin: '0 auto' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 40, justifyContent: 'space-between' }}>
                    ...
                    <div style={{ 
                        ...
                        borderRadius: 20, background: apiService.isLocalMode() ? '#f1f5f9' : '#dcfce7',
                        border: `1px solid ${apiService.isLocalMode() ? '#e2e8f0' : '#bbf7d0'}`
                    }}>
                        <div style={{ 
                            width: 8, height: 8, borderRadius: '50%', 
                            background: apiService.isLocalMode() ? '#94a3b8' : '#22c55e' 
                        }} />
                        <span style={{ fontSize: 12, fontWeight: 500, color: apiService.isLocalMode() ? '#64748b' : '#166534' }}>
                            {apiService.getProviderName()}
                            {apiService.isLocalMode() && ' (Dev Mode)'}
                        </span>
                    </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24, marginBottom: 48 }}>
                    ...
                    <div style={{
                        ...
                        {isCreating ? (
                            ...
                            >
                                \{loading ? 'Loading...' : 'Create Blank Project'}
                            </button>
                        )}
                    </div>
                ...
                {loading && projects.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: 48 }}>
                        <Loader2 size={32} style={{ color: '#3b82f6', margin: '0 auto 16px', animation: 'spin 2s linear infinite' }} />
                        <p style={{ color: '#64748b' }}>Fetching projects...</p>
                    </div>
                ) : error === 'FAILED_TO_LOAD' ? (
                    <div style={{ textAlign: 'center', padding: 48, background: '#fef2f2', borderRadius: 12, border: '1px solid #fecaca' }}>
                        <AlertCircle size={32} style={{ color: '#ef4444', margin: '0 auto 16px' }} />
                        <p style={{ color: '#991b1b', marginBottom: 16 }}>Failed to connect to the backend server.</p>
                        <button onClick={fetchProjects} style={{ background: '#ef4444', color: 'white', border: 'none', padding: '8px 16px', borderRadius: 6, cursor: 'pointer' }}>Retry</button>
                    </div>
                ) : projects.length === 0 ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 12 }}>
                        {projects.sort((a, b) => new Date(b.lastAccessed || 0).getTime() - new Date(a.lastAccessed || 0).getTime()).map(project => (
                            <div
                                key={project.id}
                                onClick={() => {
                            ...
                            >
                                <div>
                                    ...
                                    <span style={{ fontSize: 12, color: '#94a3b8' }}>Last opened: {project.lastAccessed ? new Date(project.lastAccessed).toLocaleDateString() : 'Never'}</span>
                                </div>
