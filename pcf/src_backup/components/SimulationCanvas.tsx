...
const SimulationCanvasInner = ({
    ...
    useEffect(() => {
        setNodes(nds => {
            ...
            const newNodes = nds.map(node => {
                ...
                if (node.data._kpiRef === kpi && 
                    ...
                    node.data.onCommentChange === onCommentChange) {
                    return node;
                }
        ...
        calculatedScenarioValues, selectedScenarioIds, scenarios, onCommentChange]);
...
const SimulationCanvas: React.FC<SimulationCanvasProps> = ({ projectId, isSampleMode, onBack }) => {
    ...
    const setKpis = useCallback((updater: (prev: Record<string, KPIData>) => Record<string, KPIData>, forceUpdateBase: boolean = false, targetScenarioId?: string) => {
        setAppState(prev => {
            ...
            const scenario = prev.scenarios[activeId];

            // If we're editing 'base' OR a 'promoted' scenario, we must branch into a new scenario
            // to keep the original data pristine and read-only.
            if ((activeId === 'base' || scenario?.isPromoted) && !forceUpdateBase) {
                const newId = `scenario-${Date.now()}`;
                const baseName = scenario ? scenario.name : "Scenario";
                const suffix = scenario?.isPromoted ? " (Edit)" : "";
                
                // For promoted scenarios, we name it "Original Name (Edit)"
                const newName = scenario?.isPromoted 
                    ? `${baseName}${suffix}`
                    : (() => {
                ...
                const nextSelected = prev.spreadsheetSelectedScenarios 
                    ? prev.spreadsheetSelectedScenarios.map((id: string) => id === activeId ? newId : id)
                    : [newId];
                ...
                return {
                    ...
                    scenarios: {
                        ...
                        [newId]: { 
                            id: newId, 
                            name: newName, 
                            kpis: updater(JSON.parse(JSON.stringify(prev.scenarios[activeId].kpis))), 
                            createdAt: new Date().toISOString(),
                            isPromoted: false // New clone is editable
                        }
                    },
