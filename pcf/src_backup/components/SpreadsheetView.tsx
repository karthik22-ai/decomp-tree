...
import { Save, Layers, Database, Calculator, Download, Upload, Lock, Unlock, Plus, Search, ChevronDown, Trash2, MessageSquare, X } from 'lucide-react';
...
const SpreadsheetView: React.FC<SpreadsheetViewProps> = ({
    ...
    const handleCommentClick = (kpiId: string, currentComment: string = '') => {
        setCommentingKpiId(kpiId);
        setCommentText(currentComment);
    };

    const handleCommentSave = () => {
        if (commentingKpiId && onCommentChange) {
        ...
        <div className="spreadsheet-container" onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
                                                    ...
                                                    <div className="flex items-center gap-1 ml-auto pr-2">
                                                        {kpi.comment && (
                                                            <MessageSquare 
                                                                ...
                                                                onClick={(e) => { e.stopPropagation(); handleCommentClick(kpi.id, kpi.comment); }}
                                                            />
                                                        )}
                                                        {!isVirtualScenario && (
                                                            <button 
                                                                className="comment-btn opacity-0 group-hover:opacity-100 transition-opacity"
                                                                onClick={(e) => { e.stopPropagation(); handleCommentClick(kpi.id, kpi.comment); }}
            ...
            {commentingKpiId && (
                <div 
                    ...
                    onClick={() => setCommentingKpiId(null)}
                >
                    <div 
                        style={{
                            background: 'white',
                    ...
                    >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                ...
                                <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 600, color: '#1e293b' }}>KPI Comment</h3>
                            </div>
                            <button 
                                onClick={() => setCommentingKpiId(null)}
                                style={{ background: 'none', border: 'none', padding: 4, cursor: 'pointer', color: '#94a3b8', display: 'flex' }}
                            >
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                            <button 
                                onClick={() => setCommentingKpiId(null)}
                                style={{ padding: '10px 20px', borderRadius: '8px', border: '1px solid #cbd5e1', background: 'white', color: '#64748b', fontWeight: 600, cursor: 'pointer' }}
                            >
                                Cancel
                            </button>
                            <button 
                                onClick={handleCommentSave}
                                style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', background: '#3b82f6', color: 'white', fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 6px -1px rgba(59, 130, 246, 0.3)' }}
                            >
                                Save Comment
                            </button>
                        </div>
