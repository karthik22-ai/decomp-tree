        ...
        <header className="app-header">
            ...
            <div className="header-center" style={{ flex: 1, display: 'flex', justifyContent: 'center', paddingLeft: '20px' }}>
                <nav className="header-nav" style={{ marginRight: '16px' }}>
                    <button onClick={() => handleNavClick("/")} className={currentView === "/" ? "nav-link active" : "nav-link"} style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left", font: "inherit" }}>
                        Tree<br/>View
                    </button>
                    <button onClick={() => handleNavClick("/tabular")} className={currentView === "/tabular" ? "nav-link active" : "nav-link"} style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left", font: "inherit" }}>
                        Tabular<br/>View
                    </button>
                    <button onClick={() => handleNavClick("/raw-data")} className={currentView === "/raw-data" ? "nav-link active" : "nav-link"} style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left", font: "inherit" }}>
                        Raw Data<br/>Viewer
                    </button>
                    <button onClick={() => handleNavClick("/compare")} className={currentView === "/compare" ? "nav-link active" : "nav-link"} style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left", font: "inherit" }}>
                        Compare<br/>View
                    </button>
                    <button onClick={() => handleNavClick("/logs")} className={currentView === "/logs" ? "nav-link active" : "nav-link"} style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left", font: "inherit" }}>
                        Activity<br/>Log
                    </button>
                </nav>
                <div className="date-picker-wrapper" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', width: 'auto', flexShrink: 1, padding: '0 4px' }}>
                    {onDateRangeModeChange && (
                    ...
                    <div className="date-picker-bar" style={{ background: 'white', border: '1px solid #cbd5e1', display: 'flex', alignItems: 'center', borderRadius: '6px', padding: '2px 4px' }}>
                        ...
                        <select
                            ...
                            style={{ marginLeft: '4px', minWidth: '40px', maxWidth: '60px', fontSize: '12px', background: 'transparent', color: '#1e293b', border: 'none', outline: 'none', cursor: 'pointer', fontWeight: 500 }}
                        >
                            {months.map((m, i) => <option key={i} value={i} style={{ color: 'black' }}>{m.substring(0, 3)}</option>)}
                        </select>
                        <select
                            className="date-select"
                            ...
                            style={{ minWidth: '50px', maxWidth: '60px', fontSize: '12px', background: 'transparent', color: '#1e293b', border: 'none', outline: 'none', cursor: 'pointer', fontWeight: 500 }}
                        >
                        ...
                        {dateRangeMode === 'YTD' && (
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                <span style={{ margin: '0 4px', color: '#94a3b8', fontWeight: 900 }}>—</span>
                                <select
                                    className="date-select"
                                    value={dateRange.endMonth}
                                    onChange={(e) => onDateRangeChange({ ...dateRange, endMonth: parseInt(e.target.value) })}
                                    style={{ minWidth: '40px', maxWidth: '60px', fontSize: '12px', background: 'transparent', color: '#1e293b', border: 'none', outline: 'none', cursor: 'pointer', fontWeight: 500 }}
                                >
                                    {months.map((m, i) => <option key={i} value={i} style={{ color: 'black' }}>{m.substring(0, 3)}</option>)}
                                </select>
                                <select
            ...
            <div className="header-right" style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingRight: '16px', flexWrap: 'nowrap', overflowX: 'auto' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <div className="value-type-selector">
                        <select 
                            ...
                            style={{ padding: '2px 24px 2px 8px', fontSize: '11px', height: '24px', minHeight: '24px', maxWidth: '85px' }}
                            title="Value Display Mode"
                        >
