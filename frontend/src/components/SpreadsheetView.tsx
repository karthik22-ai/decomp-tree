...
const SpreadsheetView: React.FC<SpreadsheetViewProps> = ({
    ...
    const getOverallDisplayValue = (kpi: KPIData): string => {
        ...
        // If multiple scenarios are selected, show the aggregate sum of the visible range
        if (!scenId && selectedScenarioIds.length > 1) {
            let sum = 0;
            selectedScenarioIds.forEach(id => {
                const resultsMap = id === activeScenarioId ? calculatedValues 
                                 : id === baselineScenarioId ? baseValues
                                 : (calculatedScenarioValues?.[id] as any);
                
                if (resultsMap && resultsMap[realKpiId]) {
                    sum += (resultsMap[realKpiId]?.[months.length] ?? 0);
                }
            });
            return fmt(sum);
        }
                ...
                <table className="sheet-table" style={{ borderCollapse: 'separate', borderSpacing: 0, userSelect: isDragging ? 'none' : undefined }}>
                    <thead style={{ position: 'sticky', top: 0, zIndex: 10, background: '#f8fafc' }}>
                        <tr>
                            ...
                            <th className="total-head month-column" style={{ borderBottom: '2px solid #cbd5e1' }}>Overall</th>
                            {onDateRangeChange && (
